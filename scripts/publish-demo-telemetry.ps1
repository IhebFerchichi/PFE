...
function New-VoltageSeries {
    ...
    for ($i = 0; $i -lt 16; $i++) {
        ...
        $value = [math]::Round($BaseVoltage + $noise, 3)
        $values.Add($value)
    }
...
function New-Payload {
    ...
    $temps = New-Temperatures -BaseTemp $BaseTempC -Scenario $Scenario
    $stackV = [math]::Round((($cells | Measure-Object -Sum).Sum), 3)
    $currentA = [math]::Round($BaseCurrentA + (Get-RandomOffset -Amplitude 1.1), 2)
...
Write-Host "Publishing scenario '$Scenario' to mqtt://$BrokerHost`:$BrokerPort"

for ($iteration = 1; $iteration -le $Iterations; $iteration++) {
    foreach ($package in $packages) {
        $lfpPayload = New-Payload `
            -PackType "LFP" `
            -BmsId $package.LfpBmsId `
            -BaseCellVoltage $package.LfpBaseCellV `
            -BaseCurrentA $package.BaseCurrentA `
            -BaseTempC $package.BaseTempC `
            -Soc $package.Soc `
            -Scenario $Scenario

        $supercapPayload = New-Payload `
            -PackType "SUPERCAP" `
            -BmsId $package.SupercapBmsId `
            -BaseCellVoltage $package.SupercapBaseCellV `
            -BaseCurrentA (-1 * $package.BaseCurrentA) `
            -BaseTempC ($package.BaseTempC + 1.2) `
            -Soc $package.Soc `
            -Scenario $Scenario
        ...
        Write-Host ("[{0}/{1}] Published {2}: LFP={3}, SUPERCAP={4}" -f $iteration, $Iterations, $package.Name, $package.LfpBmsId, $package.SupercapBmsId)
    }
